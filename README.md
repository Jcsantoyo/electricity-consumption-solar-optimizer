...
This project builds a complete data science pipeline for residential solar and battery optimization.
...
GitHub Actions automatically runs the test suite and Ruff lint checks on pushes and pull requests.
...
The project uses GitHub Actions to automatically run code quality checks and tests on each push and pull request.
...
This helps ensure that the project remains reproducible and that future changes do not break the main functionality.

The status badge at the top of this README shows whether the latest workflow run is passing or failing.
