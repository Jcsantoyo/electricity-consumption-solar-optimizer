...
def main() -> None:
    ...
    optimization_results_path = (
        config.OUTPUT_PATHS.forecast_optimization_results
    )
    ...
    best_self_sufficiency_scenario = (
        get_best_scenario_by_self_sufficiency(results_df)
    )
    ...
    best_scenarios_path = (
        config.OUTPUT_PATHS.forecast_optimization_best_scenarios
    )
    ...
    print(
        "Solar peak power: "
        f"{best_payback_scenario['solar_peak_power_kw']:.2f} kW"
    )
    print(
        "Battery capacity: "
        f"{best_payback_scenario['battery_capacity_kwh']:.2f} kWh"
    )
    print(
        "Annual savings: "
        f"{best_payback_scenario['annual_savings_eur']:.2f} EUR/year"
    )
    print(f"Payback: {best_payback_scenario['payback_years']:.2f} years")
    print(
        "Self-sufficiency: "
        f"{best_payback_scenario['self_sufficiency'] * 100:.2f}%"
    )
