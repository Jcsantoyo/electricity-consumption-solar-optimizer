...
REQUIRED_HOURLY_PRICE_COLUMNS = {"datetime", "price_eur_per_kwh"}


def load_hourly_prices(
    file_path: str, allow_negative_prices: bool = False
) -> pd.DataFrame:
...
def validate_hourly_price_columns(price_df: pd.DataFrame) -> None:
    ...
    if missing_columns:
        raise ValueError(
            f"Missing required hourly price columns: {sorted(missing_columns)}"
        )
...
def load_hourly_prices_if_enabled(
    use_hourly_price_data: bool, file_path: str, allow_negative_prices: bool = False
) -> pd.DataFrame | None:
...
def validate_hourly_price_coverage(
    consumption_df: pd.DataFrame, price_df: pd.DataFrame
) -> None:
    ...
    price_datetimes = set(pd.to_datetime(price_df["datetime"], errors="coerce"))
    ...
    if missing_datetimes:
        first_missing = min(missing_datetimes)
        last_missing = max(missing_datetimes)
