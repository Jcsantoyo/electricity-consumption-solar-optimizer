...
def select_consumption_period(
    consumption_df: pd.DataFrame, number_of_days: int
) -> pd.DataFrame:
    ...
    if len(consumption_df) < number_of_hours:
        raise ValueError(
            f"Consumption data does not contain enough rows for {number_of_days} days"
        )
...
def remap_consumption_timestamps(
    consumption_df: pd.DataFrame, target_start_datetime: str
) -> pd.DataFrame:
    validate_consumption_dataframe(consumption_df)
    ...
    remapped_df["datetime"] = pd.date_range(
        start=target_start, periods=len(remapped_df), freq="h"
    )
...
def build_recent_consumption_scenario(
    ...
    selected_df = select_consumption_period(consumption_df, number_of_days)

    remapped_df = remap_consumption_timestamps(selected_df, target_start_datetime)
...
def validate_consumption_dataframe(consumption_df: pd.DataFrame) -> None:
    ...
    if missing_columns:
        raise ValueError(
            f"Missing required consumption columns: {sorted(missing_columns)}"
        )
