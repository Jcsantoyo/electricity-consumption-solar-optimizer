...
def test_fixed_price_model_compensates_surplus() -> None:
    ...
    expected_import_cost = 3.0 * 0.20

    expected_surplus_income = 2.0 * 0.05

    expected_net_cost = expected_import_cost - expected_surplus_income

    assert result == pytest.approx(expected_net_cost)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            -0.01,
            ("Fixed electricity price cannot be negative"),
        ),
        (
            "surplus_compensation_price",
            -0.01,
            ("Surplus compensation price cannot be negative"),
        ),
        (
            "contracted_power_kw",
            -1.0,
            ("Contracted power cannot be negative"),
        ),
        (
            "power_price_eur_per_kw_year",
            -1.0,
            ("Annual power price cannot be negative"),
        ),
    ],
)
...
def test_fixed_price_model_rejects_invalid_simulation_days() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Simulation days must be greater than zero"),
    ):
...
def test_fixed_price_model_matches_protocol() -> None:
    ...
    accepted_model = accepts_electricity_price_model(model)
...
def test_time_of_use_price_model_calculates_net_cost() -> None:
    ...
    assert result == pytest.approx(0.25 + 0.18 + 0.12)
...
def test_hourly_price_model_calculates_net_cost() -> None:
    ...
    expected_cost = 1.0 * 0.10 + 2.0 * 0.20
...
def test_hourly_price_model_compensates_surplus() -> None:
    ...
    expected_import_cost = 2.0 * 0.20

    expected_surplus_income = 2.0 * 0.05

    assert result == pytest.approx(expected_import_cost - expected_surplus_income)
...
def test_hourly_price_model_rejects_negative_prices_by_default() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Hourly price data contains negative prices"),
    ):
        HourlyPriceModel(
            price_df=price_df,
            surplus_compensation_price=0.0,
...
def test_hourly_price_model_rejects_missing_price_timestamp() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=("Missing hourly electricity prices"),
    ):
        model.calculate_net_cost(
            energy_df=energy_df,
            grid_import_column=("grid_import_kwh"),
...
def test_hourly_price_model_supports_custom_grid_import_column() -> None:
    energy_df = pd.DataFrame(
        {
            "datetime": pd.to_datetime(
                [
    ...
    result = model.calculate_variable_grid_cost(
        energy_df=energy_df,
        grid_import_column=("custom_grid_import_kwh"),
    )

    assert result == pytest.approx(0.50)


def test_hourly_price_model_rejects_missing_energy_column() -> None:
    energy_df = pd.DataFrame(
        {
            "datetime": pd.to_datetime(
                [
    ...
    with pytest.raises(
        ...
        model.calculate_variable_grid_cost(
            energy_df=energy_df,
            grid_import_column=("grid_import_kwh"),
        )
