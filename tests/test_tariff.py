...
def test_surplus_compensation():
    df = pd.DataFrame({"solar_surplus_kwh": [10.0, 5.0]})

    compensation = calculate_surplus_compensation(
        df, surplus_column="solar_surplus_kwh", surplus_compensation_price=0.07
    )
...
def test_fixed_power_cost():
    cost = calculate_fixed_power_cost(
        contracted_power_kw=4.6, power_price_eur_per_kw_year=35.0, simulation_days=365
    )
