...
def test_omie_project_scenario_uses_hourly_prices() -> None:
    scenario = get_project_scenario("uci_omie_june_2026")
...
def test_fixed_project_scenario_has_no_hourly_file() -> None:
    scenario = get_project_scenario("uci_fixed_tariff")

    assert scenario.uses_hourly_prices is False
    assert scenario.hourly_price_data_path is None
    assert scenario.tariff_profile_name == "flat_price"
...
def test_unknown_project_scenario_raises_error() -> None:
    with pytest.raises(
        ...
        get_project_scenario("does_not_exist")


def test_default_project_scenario_name_is_registered() -> None:
    scenario = get_project_scenario(DEFAULT_PROJECT_SCENARIO_NAME)
