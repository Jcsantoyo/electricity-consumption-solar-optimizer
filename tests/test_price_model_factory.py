...
def test_factory_builds_fixed_price_model() -> None:
    ...
    assert model.fixed_price_eur_per_kwh == (
        pytest.approx(0.20)
    )

    assert model.surplus_compensation_price == (
        pytest.approx(0.07)
    )

    assert model.contracted_power_kw == (
        pytest.approx(4.6)
    )

    assert model.power_price_eur_per_kw_year == (
        pytest.approx(35.0)
    )
...
def test_factory_rejects_unsupported_price_mode() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=(
            "Unsupported electricity price mode: "
            "unknown"
        ),
    ):
        build_electricity_price_model(
...
def test_factory_builds_time_of_use_price_model() -> None:
    ...
    assert model.peak_price_eur_per_kwh == (
        pytest.approx(0.25)
    )

    assert model.flat_price_eur_per_kwh == (
        pytest.approx(0.20)
    )

    assert model.off_peak_price_eur_per_kwh == (
        pytest.approx(0.12)
    )
