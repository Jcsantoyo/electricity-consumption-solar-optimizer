...
def test_get_pvgis_generation_for_timestamps_scales_peak_power() -> None:
    ...
    timestamps = pd.Series(
        [pd.Timestamp("2025-01-01 10:00:00"), pd.Timestamp("2025-01-01 11:00:00")]
    )

    solar_generation = get_pvgis_generation_for_timestamps(
        pvgis_df, timestamps, peak_power_kw=2.0
    )
...
def test_get_pvgis_generation_for_timestamps_matches_by_month_day_hour() -> None:
    ...
    timestamps = pd.Series(
        [pd.Timestamp("2025-01-01 12:00:00"), pd.Timestamp("2025-01-02 12:00:00")]
    )

    solar_generation = get_pvgis_generation_for_timestamps(
        pvgis_df, timestamps, peak_power_kw=1.5
    )
...
def test_get_pvgis_generation_for_missing_timestamp_returns_zero() -> None:
    pvgis_df = pd.DataFrame(
        {
            "datetime": [pd.Timestamp("2020-01-01 12:10:00")],
            "solar_generation_1kw_kwh": [0.7],
        }
    )

    timestamps = pd.Series(
        [pd.Timestamp("2025-01-01 12:00:00"), pd.Timestamp("2025-01-01 13:00:00")]
    )

    solar_generation = get_pvgis_generation_for_timestamps(
        pvgis_df, timestamps, peak_power_kw=1.0
    )
